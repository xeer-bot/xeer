...
export async function deployGuildCommands() {
    ...
    try {
        log.info(`Started deploying ${bot.commands.size} guild application (/) commands.`);
...
export async function deleteGuildCommands() {
    ...
    const rest = new REST({ version: "10" }).setToken(process.env.BOT_TOKEN || "");
    try {
        log.info(`Started deleting ${bot.commands.size} guild application (/) commands.`);
        const data = await rest.put(
            Routes.applicationGuildCommands(bot.application.id, config.guildID),
            { body: [] },
        ) as { length: number };
...
export async function deleteCommands() {
    if (!process.env.BOT_TOKEN || !bot.application?.id) { log.error("Couldn't remove commands due to no token or Application ID present."); return; }
    const rest = new REST({ version: "10" }).setToken(process.env.BOT_TOKEN || "");
    try {
        log.info(`Started deleting ${bot.commands.size} application (/) commands.`);
        const data = await rest.put(
            Routes.applicationCommands(bot.application.id),
            { body: [] },
        ) as { length: number };
