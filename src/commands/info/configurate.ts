...
export class ConfigurateCommand {
	@Slash({
		name: 'configurate',
		description:
			"Configure the Welcome/Leave Messages and their Channel's IDs",
	})
	async execute(
		...
		@SlashOption({
			name: 'text',
			description:
				'Text (Placeholders: {user}, {guildname}, {timestamp})',
			required: true,
			type: ApplicationCommandOptionType.String,
		})
		text: string,
				...
				if (text != 'allow' && text != 'disallow') {
					await interaction.followUp({
						embeds: [
							errEmbed(
								new Error(),
								'Wrong text, I only accept `ALLOW` or `DISALLOW`.'
							),
						],
					});
					return;
