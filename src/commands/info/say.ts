...
export class SayCommand {
    @Slash({ name: "say", description: "Sends a message." })
    async execute(
        @SlashOption({
            name: "text",
    ...
    interaction: CommandInteraction, bot: Client): Promise<void> {
        ...
        if (cID) {
            ...
            if (chnl?.type != ChannelType.GuildText) { await interaction.followUp({ embeds: [errEmbed(new Error(), "Channel is not text-based!")] }); return; }
            let user = null;
            const gidxd = chnl.guild;
            user = await gidxd.members.fetch(interaction.user.id);
            if (!user) { await interaction.followUp({ embeds: [errEmbed(new Error(), "You're not on this guild!")] }); return; }
        }
        if (text.includes("@everyone") || text.includes("@here")) { await interaction.followUp({ embeds: [errEmbed(new Error(), "You wish :3333")] }); return; }
        ...
        if (interaction.user.id == config.ownerID) {
            ...
            if (guild?.sendcmd_toggled == "allow") {
                ...
                await interaction.followUp({ embeds: [errEmbed(new Error(), "The guild doesn't allow `say` command!")] })
            }
        } else {
            await interaction.followUp({ embeds: [npEmbed("You're not on the list of premium users!", undefined)] });
        }
    }
}
