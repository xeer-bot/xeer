	...
	async execute(
		...
		if (cID) {
			...
			if (chnl?.type != ChannelType.GuildText) {
				await interaction.followUp({
					embeds: [
						errEmbed(new Error(), 'Channel is not text-based!'),
					],
				});
				return;
			}
			...
			if (!user) {
				await interaction.followUp({
					embeds: [
						errEmbed(new Error(), "You're not on this guild!"),
					],
				});
				return;
			}
		...
		if (interaction.user.id == config.ownerID) {
			...
			if (guild?.sendcmd_toggled == 'allow') {
				...
				await interaction.followUp({
					embeds: [
						errEmbed(
							new Error(),
							"The guild doesn't allow `say` command!"
						),
					],
				});
			}
			...
			await interaction.followUp({
				embeds: [
					npEmbed(
						"You're not on the list of premium users!",
						undefined
					),
				],
			});
