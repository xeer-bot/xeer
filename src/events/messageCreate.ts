...
export class MessageCreate {
    ...
    async messageCreate(
        ...
        try {
            ...
            if (msg.author.id != config.ownerID) { await msg.reply({ embeds: [npEmbed("You're not an owner of this bot!", undefined)] }); return; }
            if (msg.content.includes("@everyone") || msg.content.includes("@here")) { await msg.reply({ embeds: [errEmbed(new Error("You wish :3333"), undefined)] }); return; }
            ...
            await msg.reply({ embeds: [errEmbed(new Error(), "I ate tacobell and crashed! :(")] })
        }
    }
}